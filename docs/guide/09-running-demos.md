...
[Back to Guide Index](../../README.md#guide) | [Previous: Brale Integration Mapping](08-brale-integration-mapping.md)

---

Both demos are self-contained binaries that exercise the full Miden SDK integration.
No prior setup is needed beyond building the project — each run starts from a clean
temp directory.
...
| `make full-demo-private` | E2E single-signer flow with **private** wallets | Miden testnet |
| `make multisig-demo` | 2-of-3 ECDSA K256 multisig flow | Miden testnet + PSM server |

---

## Storage Mode: `--private` Flag

The full demo supports two wallet storage modes, controlled by the `--private` CLI flag:

```bash
# Public wallets (default) — full state on-chain, visible in explorers
make full-demo-public

# Private wallets — only commitment hash on-chain (40 bytes)
make full-demo-private
```

| Mode | Flag | On-Chain State | Use Case |
|---|---|---|---|
| **Public** | (default) | Full account state | Explorer visibility, debugging, auditing |
| **Private** | `--private` | 40-byte commitment only | Production wallets, balance privacy |

Issuers (faucets) are always public regardless of the flag — total supply must be
auditable. Only wallet accounts are affected.
...
The full demo exercises the complete stablecoin lifecycle through a single `BraleKeystore`
client backed by a `SimulatedMpcSigner`. All three accounts (issuer + two wallets) share
one client instance, simulating a custodian managing multiple accounts.
...
The issuer account acts as both the token definition and the minting authority. On Miden,
faucets are first-class account types that can mint and burn their associated token.

### Step 2: Create Wallets

Creates two regular wallet accounts (Wallet A and Wallet B), each with their own ECDSA
K256 keypair. Storage mode depends on the `--private` flag (see above).

After creation, the client calls `sync_state()` to register note tags for the new
accounts. This ensures the client will detect notes addressed to these wallets in
subsequent syncs.
...
Reads the fungible asset vault of each wallet account from the local store. No network
call needed — balances are tracked locally after each transaction.

### Step 6: Read Total Supply

Reads the issuer's on-chain supply counter. This is stored in the faucet account's
storage slot and reflects the net tokens in circulation (minted minus burned).
...
1. **Wallet B sends a burn note** — creates a special note addressed to the issuer
   containing 100 tokens. This is a regular send transaction from Wallet B's perspective.
2. **Issuer consumes the burn note** — the issuer account consumes the note, which
   destroys the tokens and decrements the supply counter.

The client polls `sync_state()` between these steps, waiting for the burn note to be
included in a block before the issuer can consume it.
...
After key steps the demo prints a formatted balance table showing issuer supply and
wallet holdings:
...
The `S:` prefix on the issuer row indicates total supply (minted minus burned), not a
vault balance. Wallet rows show the actual fungible asset balance in the vault.
...
The multisig demo shows institutional custody using the PSM (Private State Manager) for
2-of-3 threshold ECDSA K256 signing. This is the pattern for treasury management where
multiple parties must authorize transactions.

### Step 1: Generate 3 ECDSA K256 Keypairs

Creates three independent `SimulatedMpcKeyManager` instances, each with its own secp256k1
keypair. In production, each keypair lives in a different custodian's MPC/HSM
infrastructure.

Each signer produces a **commitment** — a keccak256 hash of the public key encoded as a
`Word` (four Miden field elements). Commitments are used to register signers without
revealing public keys.
...
Registers all three signer commitments with PSM and creates a Miden account with
threshold auth.
...
aggregated proof that the threshold was met. Individual signatures and which signers
participated are not revealed on-chain.
...
### Signer Status Tables

During signing the demo prints a progressive status table:
...
Each row shows the signer name, whether they have signed, and a truncated commitment hash.
The threshold line updates as signatures are collected.

---

## PSM Server Setup

The multisig demo requires a running PSM server. PSM defaults to writing to `/var/psm/`
which requires root — use local paths instead:

```bash
# Clone the PSM repo
git clone https://github.com/OpenZeppelin/private-state-manager

# Run the server with local storage paths
...
Once the server is running, open another terminal and run `make multisig-demo`.
